...
$register_error = '';
$register_success = '';
$show_register = false; // Controls which panel to show on error
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'register') {
    ...
    if (empty($teacher_name) || empty($username) || empty($password)) {
        ...
        if ($stmt->num_rows > 0) {
            $register_error = 'Email นี้ถูกใช้ลงทะเบียนแล้ว';
            $show_register = true;
        } else {
            // Hash password and insert
            $hashed = password_hash($password, PASSWORD_DEFAULT);
            $insert = $conn->prepare("INSERT INTO users (username, password, teacher_name) VALUES (?, ?, ?)");
            $insert->bind_param("sss", $username, $hashed, $teacher_name);

            if ($insert->execute()) {
                $register_success = 'ลงทะเบียนสำเร็จ! กรุณาเข้าสู่ระบบ';
            } else {
                $register_error = 'เกิดข้อผิดพลาด กรุณาลองใหม่';
                $show_register = true;
            }
            $insert->close();
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'login') {
    ...
    if (empty($username) || empty($password)) {
        $login_error = 'กรุณากรอก Email และ Password';
    } else {
        $stmt = $conn->prepare("SELECT id, username, password, teacher_name FROM users WHERE username = ?");
        $stmt->bind_param("s", $username);
        $stmt->execute();
...
<head>
    ...
    <title>Login Page | Smart-Bus-Guard</title>
</head>
    <style>
        ...
        *{
        ...
        .toggle{
            ...
            height: 100%;
            background: linear-gradient(to right, #0026ff, #00249b);
            color: #fff;
            position: relative;
            left: -100%;
            height: 100%;
